...
const getOfferCount = (key: string) =>
  offers.filter(
    o => o.categoryKey.toLowerCase() === key.toLowerCase()
  ).length;
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {categories.map((cat, index) => {
            ...
            const isHovered = hoveredId === cat.id;
              ...
              const CardWrapper = cat.isLive ? Link : 'div';

              return (
                <CardWrapper
                  key={cat.id}
                  {...(cat.isLive ? { to: cat.pagePath } : {})}
                  onMouseEnter={() => setHoveredId(cat.id)}
                  onMouseLeave={() => setHoveredId(null)}
                  className={`group bg-white rounded-2xl shadow-sm transition-all duration-500
                    ${cat.isLive ? 'hover:shadow-xl cursor-pointer' : 'cursor-not-allowed'}
                    ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}
                  `}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <p className="text-gray-600 mb-8">
                Our curated categories help you find the best deals faster. Whether you are looking for travel, fashion, or electronics, we have got you covered.
              </p>
